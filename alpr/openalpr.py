...
class Alpr():
    ...
    CONFIG_FILE=os.path.join(_DIR, "openalpr.conf")
    RUNTIME_DIR=os.path.join(_DIR, "runtime_data")
    OPENALPRPY_LIB=(ctypes.util.find_library('openalprpy') or 
                    os.path.join(_DIR, "blobs", "libopenalprpy.so.2")) #LINUX
    
    def __init__(self, country='eu', config_file=CONFIG_FILE, runtime_dir=RUNTIME_DIR, openalprpy_lib=OPENALPRPY_LIB):
        """
        ...
        :param openalprpy_lib: The path to the OpenALPRpy bindings library.
        ...
        try:
            self._openalprpy_lib = ctypes.cdll.LoadLibrary(openalprpy_lib)
        except OSError as e:
